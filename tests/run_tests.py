...
check('Version V3.1',"APP_VERSION = '3.1.0'" in js)
check('Huit missions',len(re.findall(r"id:'(?:martroi|maison|loire|cathedrale|groslot|campo|train|cabu)'\s*,\s*title:",js))==8)
check('16 épreuves actives maximum',len(re.findall(r"m[1-8]-[a-z]+(?:-[a-z]+)?",js[js.index('const STAGE_SELECTION'):js.index('const TRANSITIONS')]))==16)
...
check('Cache versionné','3.1.0' in sw or 'v31' in sw)
...
check('Aucune dépendance applicative externe',all(x!='index.html' and x!='styles.css' and x!='sw.js' for x in external),','.join(external))
check('Lien itinéraire optionnel','google.com/maps/search' in js)
